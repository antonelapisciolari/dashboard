...
def run():
    st.set_page_config(
        page_title="Formulario Pulse 1 Semana",
        page_icon="❓",
    )
...
def create_gsheets_connection():
    try:
        conn = st.connection("gsheets_feedback_pulse_1_semana", type=GSheetsConnection)
        return conn
...
for idx in current_page_questions:
    ...
    if "options" in question_item and question_item["options"]:
        # Display as a select box or radio button
        response = st.selectbox("Seleccione una opción:", question_item["options"], key=f"response_{idx}")
    else:
        # Display as a text input
        response = st.text_input("Respuesta", key=f"response_{idx}")
